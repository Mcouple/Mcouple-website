function onDocumentMouseMove(A) { mouseX = .25 * (A.clientX - windowHalfX), mouseY = .15 * (A.clientY - windowHalfY) }

function onWindowResize(A) { camera.aspect = window.innerWidth / window.innerHeight, camera.updateProjectionMatrix(), renderer.setSize(window.innerWidth, window.innerHeight) }

function animate() { requestAnimationFrame(animate), render() }

function render() { position = .03 * ((new Date).getTime() - start_time) % 8e3, camera.position.x += .01 * (mouseX - camera.target.position.x), camera.position.y += .01 * (-mouseY - camera.target.position.y), camera.position.z = 8e3 - position, camera.target.position.x = camera.position.x, camera.target.position.y = camera.position.y, camera.target.position.z = camera.position.z - 1e3, renderer.render(scene, camera) }
App = {
    init: function() {
        container = document.createElement("div"), document.body.appendChild(container), camera = new THREE.Camera(30, window.innerWidth / window.innerHeight, 1, 3e3), camera.position.z = 6e3, scene = new THREE.Scene, geometry = new THREE.Geometry;
        ...
        A.magFilter = THREE.LinearMipMapLinearFilter, A.minFilter = THREE.LinearMipMapLinearFilter;
        var e = new THREE.Fog(4555956, -100, 3e3);
        material = new THREE.MeshShaderMaterial({ uniforms: { map: { type: "t", value: 2, texture: A }, fogColor: { type: "c", value: e.color }, fogNear: { type: "f", value: e.near }, fogFar: { type: "f", value: e.far } }, vertexShader: document.getElementById("vs").textContent, fragmentShader: document.getElementById("fs").textContent, depthTest: !1 });
        var g = new THREE.Mesh(new THREE.Plane(64, 64));
        for (i = 0; i < 8e3; i++) g.position.x = 1e3 * Math.random() - 500, g.position.y = -Math.random() * Math.random() * 200 - 15, g.position.z = i, g.rotation.z = Math.random() * Math.PI, g.scale.x = g.scale.y = Math.random() * Math.random() * 1.5 + .5, GeometryUtils.merge(geometry, g);
        mesh = new THREE.Mesh(geometry, material), scene.addObject(mesh), mesh = new THREE.Mesh(geometry, material), mesh.position.z = -8e3, scene.addObject(mesh), renderer = new THREE.WebGLRenderer({ antialias: !1 }), renderer.setSize(window.innerWidth, window.innerHeight), container.appendChild(renderer.domElement), document.addEventListener("mousemove", onDocumentMouseMove, !1), window.addEventListener("resize", onWindowResize, !1);
        var n = document.getElementById("Email");
        n.href = window.atob("bWFpbHRvOmlAaXRqb2tlci5jbg==")
    }
}, Detector.webgl || Detector.addGetWebGLMessage();
var canvas = document.createElement("canvas");
canvas.width = 32, canvas.height = window.innerHeight;
var container, context = canvas.getContext("2d"),
    gradient = context.createLinearGradient(0, 0, 0, canvas.height);
gradient.addColorStop(0, "#1e4877"), gradient.addColorStop(.5, "#4584b4"), context.fillStyle = gradient, context.fillRect(0, 0, canvas.width, canvas.height), document.body.style.background = "url(" + canvas.toDataURL("image/png") + ")";
var camera, scene, renderer, sky, mesh, geometry, material, i, h, color, sprite, size, x, y, z, colors = [],
    mouseX = 0,
    mouseY = 0,
    start_time = (new Date).getTime(),
    windowHalfX = window.innerWidth / 2,
    windowHalfY = window.innerHeight / 2;
App.init(), console.clear(), console.log("By ITJoker"), animate();